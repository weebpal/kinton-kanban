...
(function ($, PLUGIN_ID) {
  ...
  async function getCommentCount(){
    ...
    var param = await new Promise((resolve,reject)=>{
      idKanban().then(idViewKan=>{
        resolve({"_ref"  : "/k/"+kintone.mobile.app.getId()+"/?view="+idViewKan,});
      });
    });
    var test = await new Promise((resolve,reject)=>{
      kintone.api(kintone.api.url('/k/api/app/'+kintone.mobile.app.getId()+'/record/list', true), 'POST', param, function(resp) {
  ...
  function createPopUp(id){
    var markup = 
    `
    <iframe id="iframeEdit" src="${"/k/m/" + kintone.mobile.app.getId() + "/show?record="+id+ "&kanban-popup=true"}">
    </iframe>
  `
  ...
  function createPopUpAdd(){
    ...
    var markup = 
    `
    <iframe id="iframeEdit" src="${"/k/" + kintone.mobile.app.getId() + "/edit?kanban-popup=true"}">
    </iframe>
  ...
  function createPopUpEdit(id){
    ...
    var markup = 
    `
    <iframe id="iframeEdit" src="${"/k/m/" + kintone.mobile.app.getId() + "/show?record="+id+"&l.view=20&l.q&l.next=37&l.prev=0#mode=edit"}">
    </iframe>
  ...
  kintone.events.on("mobile.app.record.create.submit.success", function(ev) {
  ...
  kintone.events.on("mobile.app.record.edit.submit.success", function(event) {
    ...
    //   idKanban().then(value=>{
    //     window.location.href = "/k/" + kintone.mobile.app.getId() + "/?view="+value;
    //   })
    // }else{
  ...
  kintone.events.on("mobile.app.record.index.show",function (events) {
