...
//  b();
  ...
  function buildColumn(nameStatus,listRecord){
    ...
    (element) => {
      ...
      enDateNew.setHours(enDateNew.getHours() - 15);
      var notification = "";
      if(endDate != "" && endDate != undefined){
        if(moment().valueOf() >= enDateNew){
          count_due[nameStatus.name] += 1;
          notification = `<div class="views-field views-field-duetime"><i class="fa fa-info-circle" aria-hidden="true"></i>Due : <span>${moment(enDateNew).format('MM/DD/YYYY H:mm:ss')}</span></div>`
        }
      }  
      ...
      markup += `<div class="views-row card-${color}" data-id="${element.$id.value}" form-id="${element.field_id.value}" recurring="${element.field_recurring.value.length != 0 ? true : false}">
...
//  getCopyAppData(166);
  ...
  async function addDataToCurrentApp(data) {
    var wData = {};
    var fileInf = {};
    var keyAccept = ['name','color','field_label','User_selection','Group_selection','content','field_startDate','field_endDate','field_manual_time','field_recurring','parent_id'];
  ...
  async function cloneTaskDone(status, copyId, recurring) {
  ...
  function closeCopy(){
    $(document).click(function (e) {
      if ($(e.target).closest(".url-popup").length === 0 && flag_url == true) {
          $(".url-popup").hide();
          flag_url = false;
          $("body").removeClass("modal-open");
          $(this).find('.group-button').removeAttr('style');
          id_item_edit = false;
      }
    });
  }
  ...
  function dueTaskUpCre(task) {
    ...
    if(endDate != "" && endDate != undefined){
      ...
      if(moment().valueOf() >= enDateNew && DueEle.length == 1) {
        ...
        count_due[task.Status.value] -= 1;
        $(`[data-id="${id}"] .views-field-duetime`).remove();
        DueCol.text("");
    ...
    .sortable({
      ...
      receive: function(event, ui){
        ...
        if(validate.length == 0 || isAccess){
          ...
          if($(ui.item).find('.views-field-duetime').length != 0){
          ...
          .then((value) => {
            ...
            cloneTaskDone(receive, idSender, recurring);
          })
          .catch((err) => {
  ...
  function buildTaskColumn(task,image){
    console.log(task.field_id.value);
    ...
    estimateTime = parseInt(estimateTime) == estimateTime ? estimateTime : estimateTime.toFixed(1);
    var markup = `
    <div class="views-row card-${color}" data-id="${task.$id.value}" form-id="${task.field_id.value}" recurring="${task.field_recurring.value.length != 0 ? true : false}">
      <div class="group-button">
        <div class="button-detail" data-target="#myModal" data-toggle="modal">
          <i class="far fa-file"></i>
          <span class="tooltip-text">Detail task</span>
  ...
  kintone.events.on("app.record.create.submit.success",async function(ev) {
    ...
    await cloneTaskDone(parent.created_task_status, ev.record.$id.value, ev.record.field_recurring.value[0] || []);
  ...
  kintone.events.on("app.record.detail.process.proceed",async function(event){
    ...
    console.log("proceed");
    cloneTaskDone(event.nextStatus.value, event.record.$id.value, event.record.field_recurring.value );
  })
      ...
      var buildViewMarkup = async function (status) {
        ...
        Promise.all(arrActions)
          ...
          .then(function () {
            ...
            $("#kanban-view").append(`<div class="url-popup popup" style="display:none;">
            ...
            $(function() {
              ...
              socket.on('delete',function(dataId){
                ...
                if($(taskDel).find('.views-field-duetime').length != 0){
              ...
              socket.on('move',function(objectMove){
                ...
                if($(`[data-id=${objectMove.idTask}] .views-field-duetime`).length != 0){
              ...
              socket.on('updatestatus', async (taskdDet) => {
                ...
                if($(`[data-id=${taskdDet.idTask}] .views-field-duetime`).length != 0){
              ...
              $("#myModal").on('hidden.bs.modal',async function(event){
                ...
                if(created_task_status && created_task_id){
                  ...
                  // listRecordByStatus.push(getListRecordByStatus(parent.list_status[number_column_status[clone_current_column]].name));
                  ...
                  var listCountCommentNew  = await getCommentCount();
                  // var list_sts = await getListRecordByStatus(clone_current_column);
